<?php
    ...
    if ($_POST['book_id']) {
        //更新日時の表示
        $sql = 'UPDATE books SET created_at=CURRENT_TIMESTAMP WHERE id=?';
        $statement = mysqli_prepare($database, $sql);                
        mysqli_stmt_bind_param($statement, 'i', $_POST['book_id']);  
        mysqli_stmt_execute($statement);                             
        mysqli_stmt_close($statement);
    ...
    elseif ($_POST['submit_book_held']) {
        // 保留へ変更
        $sql = 'UPDATE books SET status="held" WHERE id=?';
        $statement = mysqli_prepare($database, $sql);
        mysqli_stmt_bind_param($statement, 'i', $_POST['book_id']);
        mysqli_stmt_execute($statement);
        mysqli_stmt_close($statement);
    ...
    // 保留数のカウント
    $sql = 'SELECT COUNT(*) as count FROM books where status = "held"';
    $result = mysqli_query($database, $sql);
    $record = mysqli_fetch_assoc($result);
    $count_held = $record['count'];
    ...
    elseif ($_POST['submit_only_held']) {
        // 保留ステータスの書籍だけを取得する
        $sql = 'SELECT * FROM books WHERE status="held" ORDER BY created_at DESC';
    }
    else {
        // 登録されている書籍を全て取得する
    ...
    <body>
        ...
        <div id="cover">
            ...
            <form action="bookshelf_index.php" method="post">
                ...
                <div class="book_status held active">
                    <input type="submit" name="submit_only_held" value="保留"><br>
                    <div class="book_count"><?php print h($count_held); ?></div>
                </div>
            </form>
        </div>
        <div class="wrapper">
                            ...
                            <div class="book_item">
                                ...
                                <div class="book_detail">
                                    ...
                                    <form action="bookshelf_index.php" method="post">
                                        ...
                                        <div class="book_status held <?php if ($status == "held") print "active"; ?>">
                                            <input type="submit" name="submit_book_held" value="保留">
                                        </div>
                                    </form>
                                    <form action="bookshelf_index.php" method="post" style="display:inline;">
                                        <div class="book_created_at">更新日時：<?php print h($created_at); ?></div>
                                        <div class="book_delete">
                                          <input type="hidden" name="book_id" value="<?php print h($id); ?>">
                                          <input type="submit" name="submit_book_delete" value="削除"><img src="images/icon_trash.png" alt="icon trash">
                                        </div>
                                    </form>
                                        <div class="book_fix">
                                            <a href="bookshelf_fix.php">修正</a><img src="images/icon_fix.png" alt="icon fix">
                                        </div>
                                </div>
                            </div>
<?php
